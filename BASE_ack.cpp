...
uint8_t * pointerToStopFlag = stopFlag;
...
int main(){
  ...
  while(1==1){ // void loop()
    while(radio.available(&pipeNo)){
      ...
      if(data[0] == 0xFF){
        printf("need to TX data\n");
        transmitData();
        
      } else {
        printf("Received another form of data\n");
      }
